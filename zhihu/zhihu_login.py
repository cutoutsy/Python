...
class ZhihuAccount(object):
    ...
    def login(self, captcha_lang: str = 'en', load_cookies: bool = True):
        ...
        if load_cookies and self.load_cookies():
            ...
            if self.check_login():
                print('登录成功')
                # r = self.session.get('https://www.zhihu.com/notifications')
                r = self.session.get('https://www.zhihu.com/follow')
                print(r.status_code)
                print(r.headers)
                print(r.url)
                # r.encoding = 'utf-8'
        ...
        if self.check_login():
            print('登录成功')
            r = self.session.get('https://www.zhihu.com/notifications', headers=headers)
            print(r.status_code)
            return True
        print('登录失败')
        return False
    ...
    def verify_login(self,):
        r = self.session.get('https://www.zhihu.com/notifications', headers=self.session.headers)
        print(r.text)
...
if __name__ == '__main__':
    account = ZhihuAccount('15664601006', 'letxxcw1z_h')
